...
from libs.MultiKeyDict import MultiKeyDict
...
class Jenkins(object):
    ...
    def maybeAddCrumb(self, req):
        ...
        if self.crumb is None:
            try:
                response = self.jenkinsOpen(requests.Request(
                    'GET', self.buildUrl(CRUMB_URL)), add_crumb=False)
                if not response:
    ...
    def maybeAddAuth(self):
        ...
        if len(self._auths) == 1:
            ...
            for name, auth in reversed(self._auths):
                try:
                    self.jenkinsOpen(
                        requests.Request('GET', self.buildUrl(INFO),
                                         auth=auth),
    ...
    def addMissingBuilds(self, data):
        ...
        response = self.jenkinsOpen(requests.Request(
            'GET', self.buildUrl(ALL_BUILDS, locals())
        ))
        if response:
    ...
    def getJobInfo(self, name, depth=0, fetch_all_builds=False):
        ...
        try:
            response = self.jenkinsOpen(requests.Request(
                'GET', self.buildUrl(JOB_INFO, locals())
            ))
            if response:
                if fetch_all_builds:
    ...
    def getJobName(self, name):
        ...
        try:
            response = self.jenkinsOpen(requests.Request(
                'GET', self.buildUrl(JOB_NAME, locals())
            ))
    ...
    def getBuildInfo(self, name, number, depth=0):
        ...
        try:
            response = self.jenkinsOpen(requests.Request(
                'GET', self.buildUrl(BUILD_INFO, locals())
            ))
            if response:
                return json.loads(response)
            else:
    ...
    def getBuildEnvVars(self, name, number, depth=0):
        ...
        try:
            response = self.jenkinsOpen(requests.Request(
                'GET', self.buildUrl(BUILD_ENV_VARS, locals())))
            if response:
                return json.loads(response)
            else:
    ...
    def getBuildTestReport(self, name, number, depth=0):
        ...
        try:
            response = self.jenkinsOpen(requests.Request(
                'GET', self.buildUrl(BUILD_TEST_REPORT, locals())))
            if response:
                return json.loads(response)
            else:
                raise JenkinsException('job[%s] number[%d] does not exist' % (name, number))
    ...
    def getQueueInfo(self):
        ...
        '''  # noqa: E501
        return json.loads(self.jenkinsOpen(
            requests.Request('GET', self.buildUrl(Q_INFO))
        ))['items']
    ...
    def cancelQueue(self, id):
        ...
        try:
            self.jenkinsOpen(
                requests.Request(
                    'POST', self.buildUrl(CANCEL_QUEUE, locals()),
    ...
    def getInfo(self, item="", query=None):
        ...
        try:
            return json.loads(self.jenkinsOpen(
                requests.Request('GET', self.buildUrl(url))
            ))
        except (req_exc.HTTPError):
    ...
    def getWhoami(self, depth=0):
        ...
        try:
            response = self.jenkinsOpen(requests.Request(
                'GET', self.buildUrl(WHOAMI_URL, locals())
            ))
            if response is None:
    ...
    def getPluginsInfo(self, depth=2):
        ...
        """
        warnings.warn("getPluginsInfo() is deprecated, use getPlugin()",
                      DeprecationWarning)
    ...
    def getPlugin(self, depth=2):
        ...
        try:
            plugins_info_json = json.loads(self.jenkinsOpen(
    ...
    def copyJob(self, from_name, to_name):
        ...
        self.jenkinsOpen(requests.Request(
            'POST', self.buildUrl(COPY_JOB, locals())
        ))
    ...
    def renameJob(self, from_name, to_name):
        ...
        self.jenkinsOpen(requests.Request(
            'POST', self.buildUrl(RENAME_JOB, locals())
        ))
    ...
    def deleteJob(self, name):
        ...
        folder_url, short_name = self.getJobFolder(name)
        self.jenkinsOpen(requests.Request(
            'POST', self.buildUrl(DELETE_JOB, locals())
        ))
    ...
    def enableJob(self, name):
        ...
        folder_url, short_name = self.getJobFolder(name)
        self.jenkinsOpen(requests.Request(
            'POST', self.buildUrl(ENABLE_JOB, locals())
        ))
    ...
    def disableJob(self, name):
        ...
        folder_url, short_name = self.getJobFolder(name)
        self.jenkinsOpen(requests.Request(
            'POST', self.buildUrl(DISABLE_JOB, locals())
        ))
    ...
    def setNextbBuildNumber(self, name, number):
        ...
        folder_url, short_name = self.getJobFolder(name)
        self.jenkinsOpen(requests.Request(
            'POST', self.buildUrl(SET_JOB_BUILD_NUMBER, locals()),
    ...
    def createJob(self, name, config_xml):
        ...
        try:
            self.jenkinsOpen(requests.Request(
                'POST', self.buildUrl(CREATE_JOB, locals()),
            ...
            if token:
                if isinstance(parameters, list):
                    ...
                    raise JenkinsException('build parameters can be a dictionary '
                                           ...
                                           'like [("param_key", "param_value",), ...]')
            return (self.buildUrl(BUILD_WITH_PARAMS_JOB, locals()) +
    ...
    def buildJob(self, name, parameters=None, token=None):
        ...
        '''
        response = self.jenkinsRequest(requests.Request(
            'POST', self.buildJobUrl(name, parameters, token)))
    ...
    def runScript(self, script, node=None):
        ...
        result = self.jenkinsOpen(requests.Request(
            'POST', url, data=data))
    ...
    def stopBuild(self, name, number):
        ...
        folder_url, short_name = self.getJobFolder(name)
        self.jenkinsOpen(requests.Request(
            'POST', self.buildUrl(STOP_BUILD, locals())
        ))
    ...
    def getRunningBuilds(self):
        ...
        for node in nodes:
            ...
            try:
                ...
                if ('[500]' in str(e) and
                        self.getNodeInfo(node_name, depth=0)):
                    continue
                else:
                    raise
    ...
    def getNodes(self, depth=0):
        ...
        '''
        try:
            nodes_data = json.loads(self.jenkinsOpen(
    ...
    def getNodeInfo(self, name, depth=0):
        ...
        try:
            response = self.jenkinsOpen(requests.Request(
                'GET', self.buildUrl(NODE_INFO, locals())
            ))
            if response:
                return json.loads(response)
            else:
    ...
    def deleteNode(self, name):
        ...
        self.getNodeInfo(name)
        self.jenkinsOpen(requests.Request(
            'POST', self.buildUrl(DELETE_NODE, locals())
        ))
    ...
    def disableNode(self, name, msg=''):
        ...
        if info['offline']:
            return
        self.jenkinsOpen(requests.Request(
            'POST', self.buildUrl(TOGGLE_OFFLINE, locals())
        ))
    ...
    def createNode(self, name, numExecutors=2, nodeDescription=None,
        ...
        self.jenkinsOpen(requests.Request(
            'POST', self.buildUrl(CREATE_NODE, locals()), data=params)
        )
    ...
    def getBuildConsoleOutput(self, name, number):
        ...
        try:
            response = self.jenkinsOpen(requests.Request(
                'GET', self.buildUrl(BUILD_CONSOLE_OUTPUT, locals())
            ))
            if response:
                return response
            else:
    ...
    def getViewJobs(self, name):
        ...
        try:
            response = self.jenkinsOpen(requests.Request(
                'GET', self.buildUrl(VIEW_JOBS, locals())
            ))
            if response:
    ...
    def getViewName(self, name):
        ...
        try:
            response = self.jenkinsOpen(requests.Request(
                'GET', self.buildUrl(VIEW_NAME, locals())))
        except NotFoundException:
            return None
        else:
            actual = json.loads(response)['name']
    ...
    def deleteView(self, name):
        ...
        folder_url, short_name = self.getJobFolder(name)
        self.jenkinsOpen(requests.Request(
            'POST', self.buildUrl(DELETE_VIEW, locals())
        ))
    ...
    def createView(self, name, config_xml):
        ...
        self.jenkinsOpen(requests.Request(
            'POST', self.buildUrl(CREATE_VIEW, locals()),
            data=config_xml.encode('utf-8'),
            headers=DEFAULT_HEADERS
        ))
    ...
    def getPromotionName(self, name, job_name):
        ...
        try:
            response = self.jenkinsOpen(requests.Request(
                'GET', self.buildUrl(PROMOTION_NAME, locals())))
        except NotFoundException:
            return None
        else:
            actual = json.loads(response)['name']
    ...
    def getPromotionsInfo(self, job_name, depth=0):
        ...
        try:
            response = self.jenkinsOpen(requests.Request(
                'GET', self.buildUrl(PROMOTION_INFO, locals())))
            if response:
                return json.loads(response)
            else:
    ...
    def deletePromotion(self, name, job_name):
        ...
        folder_url, short_name = self.getJobFolder(job_name)
        self.jenkinsOpen(requests.Request(
            'POST', self.buildUrl(DELETE_PROMOTION, locals())
        ))
    ...
    def getPromotionConfig(self, name, job_name):
        ...
        request = requests.Request(
            'GET', self.buildUrl(CONFIG_PROMOTION, locals()))
        return self.jenkinsOpen(request)
    ...
    def getCredentialInfo(self, name, folder_name, domain_name='_'):
        ...
        try:
            response = self.jenkinsOpen(requests.Request(
                'GET', self.buildUrl(CREDENTIAL_INFO, locals())
            ))
            if response:
                return json.loads(response)
            else:
    ...
    def createCredential(self, folder_name, config_xml,
        ...
        self.jenkinsOpen(requests.Request(
            'POST', self.buildUrl(CREATE_CREDENTIAL, locals()),
            data=config_xml.encode('utf-8'),
            headers=DEFAULT_HEADERS
        ))
    ...
    def listCredentials(self, folder_name, domain_name='_'):
        ...
        folder_url, short_name = self.getJobFolder(folder_name)
        response = self.jenkinsOpen(requests.Request(
            'GET', self.buildUrl(LIST_CREDENTIALS, locals())
        ))
    ...
    def waitForNormalOp(self, timeout):
        ...
        if (not self.timeoutWarningIssued and
