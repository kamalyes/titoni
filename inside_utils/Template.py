...
class Template(metaclass=TemplateMeta):
    ...
    def subStitute(self,mapping,type=None):
        def convert(mo):
        ...
        return eval(self.pattern.sub(convert, self.template)) if type ==dict else self.pattern.sub(convert, self.template).replace("'", '"').replace("None", "null")

if __name__ == '__main__':
    general_data = Loader.jsonFile("create_virtual_product.json")
    replace_str = {'category_id': 15888, 'name': '自动化-虚拟商品7eN95a',
                   ...
                   'selling_point': '', 'term_value': '2021-07-21 13:34:33', 'market_price': '', 'retail_price': '',
                   'center_pic': 'http://file.vetscloud.com/0c97a173a6c78848a8fc554f7a65eab6'}
    print(Template(general_data).subStitute(replace_str))
