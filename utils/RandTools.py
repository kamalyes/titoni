...
class Randoms(object):
    ...
    def getVerifi(self,maxnum,radcount):
        ...
        while count < radcount:
            ...
            count += 1
        self.logger.info("成功生成%s个%s位长度的验证码%s"%(radcount,maxnum,verifi_code))
        return verifi_code
...
if __name__ == '__main__':
    Randoms = Randoms()
    Randoms.getEmail(emailtype="@qq.com", maxnum=10, rad_count=5)
    Randoms.getVerifi(maxnum=6,radcount=2)
    Randoms.getPhone(radcount=6)
    Randoms.getName(length=2,radcount=10)
    Randoms.getStrList(length=5)
    Randoms.getStr(5)
    Randoms.getFloat("1,5,6")